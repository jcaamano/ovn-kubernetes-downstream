...
var (
	...
	nodeSecondaryAddr = map[string]string{
		"node": "172.100.0.1/24",
	}
)
	...
	return &corev1.Node{
		ObjectMeta: metav1.ObjectMeta{
			...
			Annotations: map[string]string{
				...
				util.OvnNodeIfAddr:         "{\"ipv4\":\"" + nodePrimaryAddr[tn.Name] + "\"}",
				util.OVNNodeHostCIDRs:      "[\"" + nodeSecondaryAddr[tn.Name] + "\"]",
			},
		},
	}
}
	...
	tests := []struct {
		...
		},
		{
			...
			nads: []*testNAD{
				...
				{Name: "red", Namespace: "red", Network: "cluster.udn.red", Topology: "layer3", Subnet: "1.2.0.0/16", Labels: map[string]string{"selected": "true"}},
				{Name: "blue", Namespace: "blue", Network: "cluster.udn.blue", Topology: "layer3", Subnet: "1.3.0.0/16"},
			},
			nodes: []*testNode{
			...
			eips: []*testEIP{
				{Name: "eip1", EIPs: map[string]string{"node": "1.0.1.1"}},
				{Name: "eip2", EIPs: map[string]string{"node": "172.100.0.16"}}, // secondary interface EIP, ignored
				{Name: "eip3", EIPs: map[string]string{"node": "1.0.1.2"}, NamespaceSelector: map[string]string{"selected": "red"}},
				{Name: "eip4", EIPs: map[string]string{"node": "1.0.1.3"}, NamespaceSelector: map[string]string{"selected": "blue"}}, // namespace served by unselected network, ignored
			},
			reconcile:            "ra",
			expectAcceptedStatus: metav1.ConditionTrue,
			expectFRRConfigs: []*testFRRConfig{
				{
					...
					NodeSelector: map[string]string{"kubernetes.io/hostname": "node"},
					Routers: []*testRouter{
						{ASN: 1, VRF: "red", Prefixes: []string{"1.0.1.1/32", "1.0.1.2/32"}, Neighbors: []*testNeighbor{
							{ASN: 1, Address: "1.0.0.100", Advertise: []string{"1.0.1.1/32", "1.0.1.2/32"}},
						}},
					}},
			},
			expectNADAnnotations: map[string]map[string]string{"default": {types.OvnRouteAdvertisementsKey: "[\"ra\"]"}},
