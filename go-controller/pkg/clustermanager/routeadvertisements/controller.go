...
func NewController(
	...
	nsConfig := &controllerutil.ControllerConfig[core.Namespace]{
		RateLimiter:    workqueue.DefaultTypedControllerRateLimiter[string](),
		Reconcile:      c.reconcileRAsAdvertisingEgressIPs,
		Threadiness:    1,
...
func (c *Controller) ReconcileNetwork(name string, old, new util.NetInfo) {
	...
	if old != nil {
		oldNamespaces.Insert(old.GetNamespaces()...)
	}
	if new != nil {
		newNamespaces.Insert(new.GetNamespaces()...)
	}
...
func (c *Controller) generateFRRConfigurations(ra *ratypes.RouteAdvertisements) ([]*frrtypes.FRRConfiguration, []*nadtypes.NetworkAttachmentDefinition, error) {
	...
	for _, nad := range nads {
		...
		namespaceSet.Insert(network.GetNamespaces()...)
	}
	// ordered
	...
	getEgressIPs := func(nodeName string) ([]string, error) {
		if nodeEgressIPs == nil {
			nodeEgressIPs, err = c.getEgressIPsSelectingNamespacesByNode(namespaceSet)
			if err != nil {
				return nil, err
			}
		}
...
// getEgressIPsSelectingNamespacesByNode iterates all existing egress IPs and
// returns them indexed by node. Egress IPs for secondary interfaces are
// ignored. Egress IPs not selecting any of the specified namespaces are ignored.
func (c *Controller) getEgressIPsSelectingNamespacesByNode(namespaces sets.Set[string]) (map[string][]string, error) {
	eips, err := c.eipLister.List(labels.Everything())
	if err != nil {
		return nil, err
	}

	secondaryNetworks := map[string][]string{}
	isEgressIPForSecondaryInterface := func(nodeName, ip string) (bool, error) {
		if secondaryNetworks[nodeName] == nil {
			node, err := c.nodeLister.Get(nodeName)
			if err != nil {
				return false, err
			}
			eipConfig, err := egressip.GetNodeEIPConfig(node)
			if err != nil {
				return false, err
			}
			secondaryNetworks[nodeName], err = egressip.GetNodeHostCIDRsExcludeOVNNetworks(node, eipConfig)
			if err != nil {
				return false, err
			}
		}
		return egressip.IsEgressIPSecondaryNetwork(secondaryNetworks[nodeName], net.ParseIP(ip))
	}

	appliesToSelectedNamespace := func(eip *eiptypes.EgressIP) (bool, error) {
	...
	for _, eip := range eips {
		// skip EIPs for namespaces not served by any of the selected networks
		applies, err := appliesToSelectedNamespace(eip)
		if err != nil {
			return nil, err
		}
		if !applies {
...
func nsNeedsUpdate(oldObj, newObj *core.Namespace) bool {
