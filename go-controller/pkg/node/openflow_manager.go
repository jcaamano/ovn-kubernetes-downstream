...
func (c *openflowManager) addNetwork(nInfo util.NetInfo, masqCTMark, pktMark uint, v6MasqIPs, v4MasqIPs *udn.MasqueradeIPs) error {
	if err := c.defaultBridge.addNetworkBridgeConfig(nInfo, masqCTMark, pktMark, v6MasqIPs, v4MasqIPs, c.watchFactory); err != nil {
		return err
	}
	if c.externalGatewayBridge != nil {
		if err := c.externalGatewayBridge.addNetworkBridgeConfig(nInfo, masqCTMark, pktMark, v6MasqIPs, v4MasqIPs, c.watchFactory); err != nil {
			return err
		}
	}
	return nil
...
func newGatewayOpenFlowManager(gwBridge, exGWBridge *bridgeConfiguration, subnets []*net.IPNet,
	...
	if err := ofm.updateBridgeFlowCache(subnets, extraIPs, isRoutingAdvertised, false); err != nil {
		return nil, err
	}
...
func (c *openflowManager) updateBridgeFlowCache(subnets []*net.IPNet, extraIPs []net.IP,
	isPodNetworkAdvertised, isUDNNetworkAdvertised bool) error {
	...
	dftCommonFlows, err := commonFlows(subnets, c.defaultBridge, isPodNetworkAdvertised, isUDNNetworkAdvertised)
	if err != nil {
		return err
	}
	...
	if c.externalGatewayBridge != nil {
		...
		exGWBridgeDftFlows, err := commonFlows(subnets, c.externalGatewayBridge, false, false)
		if err != nil {
			return err
		}
